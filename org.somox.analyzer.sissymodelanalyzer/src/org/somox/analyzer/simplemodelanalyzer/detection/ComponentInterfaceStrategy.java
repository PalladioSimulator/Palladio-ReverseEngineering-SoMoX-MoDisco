...
public class ComponentInterfaceStrategy implements IComponentInterfaceStrategy {
     ...
     */
    @Override
    public boolean isComponentInterface(final Type classToCheck) {
        return isRegularInterface(classToCheck) ||
     ...
     */
    private boolean isPureVirtualClass(final Type classToCheck) {

        // do not consider "empty" classes with no methods as interface
        if(KDMHelper.getMethods(classToCheck).size() == 0) {
            return false;
        }
        for(final MethodDeclaration method : KDMHelper.getMethods(classToCheck)) {
